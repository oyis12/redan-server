...
// // Determine dynamic folder and settings
...
//       req.uploadedFiles = req.files; // Attach uploaded files for controller
//       next();
//     } catch (cleanupErr) {
...
// Upload middleware with Cloudinary cleanup
const uploadMiddleware = async (req, res, next) => {
  upload(req, res, async (err) => {
    if (err instanceof multer.MulterError) {
      console.error('Multer Error:', err);
